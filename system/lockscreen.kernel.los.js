// system/lockscreen.kernel.js.los.js
class LockscreenKernel {
    ...
    startSyncInterval() {
        // Синхронизация каждую секунду когда локскрин виден
        setInterval(() => {
            if (this.lockscreenElement && this.lockscreenElement.style.display === 'flex') {
                this.syncWithSystemStatus();
            }
        }, 1000);
    }
...
// 🔥 УБРАНА АВТО-ИНИЦИАЛИЗАЦИЯ - теперь управляется через kernel.js
